...
const blog = {
	...
	corpus: {
		...
		/** @type {number} */
		get size(){
			return Object.keys(this.set).map(word => this.set[word]).reduce((a, b) => a + b, 0);
		},
		stats(){
			this.init();
	...
	move(diff){
		this.goto(Math.min(this.max, Math.max(0, this.current + diff)));
	},
	random(){
		this.goto(Math.floor(Math.random() * (max+1)));
	},
	...
	set(elem){
		try {
			...
			// if (this.debug && typeof notif !== "undefined")
			//	notif.catch(e);
		}
		// grey out buttons...
		if (this.current === 0)
